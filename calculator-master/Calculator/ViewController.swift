...
class ViewController: UIViewController {
    ...
    @IBAction func percentage(_ sender: UIButton) {
        let currentText = self.displayLabel.text ?? "0"
        var origin: Double = Double(currentText)!
        origin = origin / 100
        let after = "\(origin)"
        self.displayLabel.text = after
        
    }
    @IBAction func exp(_ sender: UIButton) {
        let currentText = self.displayLabel.text ?? "0"
        var origin: Double = Double(currentText)!
        origin = pow(2.718,origin)
        let after = "\(origin)"
        self.displayLabel.text = after
    ...
    @IBAction func reverseButton(_ sender: UIButton) {
        ...
        if !(currentText=="0"){
            guard !currentText.contains("-") else {
                let new = String(currentText.characters.suffix(currentText.characters.count-1))
